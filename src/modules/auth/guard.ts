...
export async function getWhitelistedUser(): Promise<User | null> {
  ...
  const { data } = await supabase.from("allowed_users").select("email").ilike("email", user.email).maybeSingle();
  return data ? user : null;
}

export async function requireWhitelistedUser() {
  const user = await getWhitelistedUser();
  if (!user) throw new Error("Brak dostępu. Skontaktuj się z administratorem.");
  return user;
}