        ...
        try {
            ...
            await admin
                ...
                .update({
                    ...
                    best_offer_url: best?.bookingUrl ?? null,
                    best_provider: best?.provider ?? null,
                })
                .eq('id', alert.id);
            const matching = offers
                .filter((offer) => offer.price <= Number(alert.max_price))
                .map((offer) => ({ alertId: alert.id, offer }));
            if (matching.length)
                dealsByUser.set(alert.user_id, [
                    ...(dealsByUser.get(alert.user_id) ?? []),
                    ...matching,
                ]);
        } catch (error) {
            console.error('Alert scan failed', {
                alertId: alert.id,
    ...
    for (const [userId, deals] of dealsByUser) {
        try {
            ...
            const { error: dispatchError } = await admin
                ...
                .insert({ user_id: userId, dispatch_date: dispatchDate });
            ...
            console.error('Daily push failed', { userId, error });
        }
    }
    return {
        scanned: alerts.length,
        usersWithDeals: dealsByUser.size,
        notifications,
    };
}
